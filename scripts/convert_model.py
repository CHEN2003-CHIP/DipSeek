...
def convert_torch2transformers_dipseek(torch_path, transformers_path, dtype=torch.float16):
    DipSeekConfig.register_for_auto_class()
    DipSeekForCausalLM.register_for_auto_class("AutoModelForCausalLM")
    lm_model = DipSeekForCausalLM(lm_config)
    ...
    print(f"模型已保存为 Transformers-DipSeek 格式: {transformers_path}")
...
def convert_merge_base_lora(base_torch_path, lora_path, merged_torch_path):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    lm_model = DipSeekForCausalLM(lm_config).to(device)
...
if __name__ == '__main__':
    lm_config = DipSeekConfig(hidden_size=768, num_hidden_layers=8, max_seq_len=8192, use_moe=False)

    # convert torch to transformers
    torch_path = f"../out/full_sft_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth"
    transformers_path = '../dipseek-3'
